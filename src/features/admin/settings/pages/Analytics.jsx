...
const Analytics = () => {
  const { user } = useAuth();
  const { data: settings, loading, updateSettings } = useSettings('analytics');
  ...
  const handleSave = async () => {
    ...
    if (result.success) {
      ...
      await logAuditEvent(user?.uid, 'UPDATE', 'analytics', oldData, formData);
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <h2 className="text-lg font-medium text-gray-900 mb-4">Tracking Options</h2>
        <div className="space-y-4">
          <Toggle
            label="Enable Page View Tracking"
            name="enablePageViews"
            checked={formData.enablePageViews !== false}
            onChange={handleChange}
          />
          <Toggle
            label="Enable Event Tracking"
            name="enableEvents"
            checked={formData.enableEvents || false}
            onChange={handleChange}
          />
          <Toggle
          ...
          <Toggle
            ...
            name="respectDNT"
            checked={formData.respectDNT !== false}
            onChange={handleChange}
          />
        </div>
      </Card>
